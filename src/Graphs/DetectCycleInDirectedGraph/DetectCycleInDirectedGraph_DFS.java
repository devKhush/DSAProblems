...
// https://www.geeksforgeeks.org/detect-cycle-direct-graph-using-visiteds/

/*
INTUITION:
    * """On the same path, node has be visited again for Cycle"""
...
public class DetectCycleInDirectedGraph_DFS {
    ...
    * int[] array checks three things:
      ...
      * COLOR 0: Vertex is not processed yet. Initially, all vertices are marked with 0. (DFS call not made yet)
      ...
      * COLOR 1: DFS call for this vertex is ongoing (implies the vertex is in Recursion call stack for DFS)
                 ...
                 Node has been visited and is present in the current ongoing path.
      * COLOR 2: DFS call for this Vertex and all its descendants are processed (implies the vertex is not in the Recursion call
                 ...
                 Node has been visited but is not present in the current ongoing path.

      As, the "Cycle will be present only if a vertex is present in the recursion stack & we found it
      again somehow in DFS Recursion stack of other adjacent Vertex". Or it was in the current path,
      and we found it again somehow.

     * Time Complexity: O(V + E)     Same as DFS for Graph with adjacency list
     * Space Complexity: O(V)        Same as DFS for Graph with adjacency list
     */

    private boolean detectCycleInDirectGraph_DFS(int vertex, ArrayList<ArrayList<Integer>> adjList, int[] visited, int V){
        visited[vertex] = 1;

        for (int adjacentVertex : adjList.get(vertex)){
            if (visited[adjacentVertex] == 0) {
                if (detectCycleInDirectGraph_DFS(adjacentVertex, adjList, visited, V))
                    return true;
            }
    ...
    public boolean detectCycle_Color_DFS(int V, ArrayList<ArrayList<Integer>> adjList){
        ...
        for (int vertex = 0; vertex < V; vertex++)
            if (visited[vertex] == 0  &&  detectCycleInDirectGraph_DFS(vertex, adjList, visited, V))
                    return true;
